...
function normalizeStore(store) {
  const nextStore = store && typeof store === "object" ? { ...store } : createDefaultStore();

  if (!Array.isArray(nextStore.users)) {
    nextStore.users = [];
  }
  if (!nextStore.predictions || typeof nextStore.predictions !== "object") {
    nextStore.predictions = {};
  }
  nextStore.globalBracket = {
    ...defaultGlobalBracket(),
    ...(nextStore.globalBracket || {}),
    entryAssignments: normalizeEntryAssignments(nextStore.globalBracket?.entryAssignments),
  };
  nextStore.settings = {
    ...defaultSettings(),
    ...(nextStore.settings || {}),
    predictionWindowOpen:
      nextStore.settings?.predictionWindowOpen !== undefined
        ? Boolean(nextStore.settings.predictionWindowOpen)
        : true,
  };
  nextStore.actualResults = {
    ...defaultActualResults(),
    ...(nextStore.actualResults || {}),
    results: normalizeResults(nextStore.actualResults?.results),
  };

  const adminExists = nextStore.users.some((user) => user.usernameKey === ADMIN_USERNAME_KEY);
  if (!adminExists) {
  ...
  nextStore.users.forEach((user) => {
    const existingPrediction = nextStore.predictions[user.usernameKey];
    nextStore.predictions[user.usernameKey] = {
...
function readFileStoreSnapshot() {
...
function writeFileStoreSnapshot(store) {
  if (!fs.existsSync(DATA_DIR)) {
    fs.mkdirSync(DATA_DIR, { recursive: true });
  }
...
async function ensurePostgresStore() {
  ...
  const existing = await dbPool.query("SELECT payload FROM app_state WHERE id = 1");
  if (existing.rowCount === 0) {
    const seedStore = normalizeStore(readFileStoreSnapshot());
    await dbPool.query(
...
async function ensureStore() {
  ...
  const store = normalizeStore(readFileStoreSnapshot());
  writeFileStoreSnapshot(store);
}
...
async function readStore() {
  ...
  if (dbPool) {
    const result = await dbPool.query("SELECT payload FROM app_state WHERE id = 1");
    return normalizeStore(result.rows[0]?.payload);
  }

  return normalizeStore(readFileStoreSnapshot());
}
...
async function writeStore(store) {
  ...
  writeFileStoreSnapshot(normalized);
}
...
async function handleApi(req, res, url) {
  if (req.method === "GET" && url.pathname === "/api/session") {
    ...
    const store = session ? session.store : await readStore();
